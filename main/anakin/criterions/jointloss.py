...
class JointsLoss(TensorLoss):
    ...
    def __init__(self, **cfg):
        ...
        self.lambda_verts_3d = cfg.get("LAMBDA_VERTS_3D", 0.0)
        ...
        logger.info(f"  |   LAMBDA_VERTS_3D : {self.lambda_verts_3d}")
    ...
    def __call__(self, preds: Dict, targs: Dict, **kwargs) -> Tuple[torch.Tensor, Dict]:
        ...
        # ============== HAND VERTS 3D MSE LOSS >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        if self.lambda_verts_3d:
            pred_hand_verts_3d_abs = preds["hand_verts_3d_abs"]
            hand_verts_3d_abs = targs[Queries.HAND_VERTS_3D]  # TENSOR(B, NCORNERS, 3)

            # mask invisible corners
            hand_vis_mask = targs[Queries.HAND_VERTS_VIS]
            pred_hand_verts_3d_abs = torch.einsum("bij,bi->bij", pred_hand_verts_3d_abs,
                                               hand_vis_mask.to(final_loss.device))
            hand_verts_3d_abs = torch.einsum("bij,bi->bij", hand_verts_3d_abs, hand_vis_mask)

            hand_verts_3d_loss = torch_f.mse_loss(pred_hand_verts_3d_abs, hand_verts_3d_abs.to(final_loss.device))
            final_loss += self.lambda_verts_3d * hand_verts_3d_loss
        else:
