...
from anakin.metrics.metric import AverageMeter
...
class Criterion(TensorLoss):
    ...
    def compute_losses(self, preds: Dict, targs: Dict, **kwargs) -> Tuple[torch.Tensor, Dict]:
        ...
        for loss in self.loss_list:
            ...
            for k, v in losses.items(): # possible duplicates in discriminator loss
                if k in total_losses.keys() and v is not None: 
                    total_losses[k] += v
                elif v is not None:
                    total_losses[k] = v
